...
class System extends Router{
	...
	private function setArea() {
		foreach($this->routers as $i => $value) {
	...
	private function setParams() {
		...
		if($this->onRaiz) {
			...
			unset($this->exploder[0], $this->exploder[1], $this->exploder[1]);
	...
	private function validarController() {
		if(!class_exists($this->runController)) {
			...
			die('Controller nao existe ' . $this->runController);
		}
	}
	...
	public function Run() {
		...
		$this->runController = 'controller\\'. $this->controller . 'Controller';
